...
# --- WBUDOWANY LOKALNY FILTR KALMANA ---
class LocalKinematicKF:
    def __init__(self):
        # Stan: [v_x (przód), v_y (bok), omega (obrót)]
        # Traktujemy przesunięcie klatka-do-klatki jako "prędkość"
        self.x = np.zeros(3)
        self.P = np.eye(3) * 0.1 # Inicjalna niepewność
    ...
    def filter_step(self, z_measured):
        # 1. Predykcja (Zakładamy stałą prędkość klatka do klatki)
        x_pred = self.x
        P_pred = self.P + self.Q
        
        # 2. Aktualizacja (Korekta pomiarem z LoFTR)
        y_innov = z_measured - x_pred            # Innowacja (błąd)
        S = P_pred + self.R                      # Kowariancja innowacji
        K = P_pred @ np.linalg.inv(S)            # Zysk Kalmana
...
# --- GŁÓWNA KLASA (TWÓJ ORYGINALNY BAZOWY KOD) ---
class sonar_odometry(nn.Module):
    ...
    def __init__(self, model_config, sonar_config, device, 
        ...
        self.kinematic_kf = LocalKinematicKF()
    ...
    def fls_filter(self, frame):
        ...
        for i in range(b):
            ...
            # Lekkie ścięcie cieni ułatwi życie
            bilateral[bilateral < 20] = 20
    ...
    # [set_init_state, _apply_spatial_bucketing, _weighted_kabsch, _match_and_estimate 
    #  POZOSTAJĄ BEZ ZMIAN Względem twojego ORYGINALNEGO KODU]
    
    # TUTAJ TYLKO WIZUALNIE SKRÓCONE DLA CZYTELNOŚCI - WKLEJ SWOJE METODY:
    ...
    def forward(self, frame, depth, return_visu=False):
        ...
        # KLASYCZNE DOPASOWANIE DO CAŁEGO OKNA
        ...
        if len(est_poses) > 0:
            ...
            # --- ZASTOSOWANIE FILTRU KALMANA (Zamiast sztywnego Damping) ---
            R_curr = self.current_pose[0:2, 0:2]
            t_curr = self.current_pose[0:2, 2]
            
            # 1. Wyciągamy surowy wektor ruchu względem układu lokalnego robota
            t_new_raw = np.array([median_x, median_y])
            delta_t_local_meas = R_curr.T @ (t_new_raw - t_curr)
            
            # 2. Wyciągamy surowy skok kąta
            prev_azimuth = np.arctan2(self.current_pose[1, 0], self.current_pose[0, 0])
            delta_theta_meas = median_azimuth - prev_azimuth
            delta_theta_meas = np.arctan2(np.sin(delta_theta_meas), np.cos(delta_theta_meas)) # Normalizacja -pi, pi
            
            # 3. Formujemy wektor pomiarowy Z i wpuszczamy do Kalmana
            z_meas = np.array([delta_t_local_meas[0], delta_t_local_meas[1], delta_theta_meas])
            filtered_deltas = self.kinematic_kf.filter_step(z_meas)
            
            # 4. Rekonstruujemy nową pozę używając PRZEFILTROWANYCH wartości
            delta_t_filtered = np.array([filtered_deltas[0], filtered_deltas[1]])
            delta_theta_filtered = filtered_deltas[2]
            
            t_new_filtered = t_curr + R_curr @ delta_t_filtered
            new_azimuth_filtered = prev_azimuth + delta_theta_filtered
            
            new_pose = np.array([
                [np.cos(new_azimuth_filtered), -np.sin(new_azimuth_filtered), t_new_filtered[0]], 
                [np.sin(new_azimuth_filtered),  np.cos(new_azimuth_filtered), t_new_filtered[1]], 
                [0,                            0,                           1]
            ])
            step_is_valid = True
        else:
            # W przypadku utraty trackingu (0 inlierów), Kalman jedzie na samej predykcji stałej prędkości
            z_meas = np.zeros(3) # Brak pomiaru
            filtered_deltas = self.kinematic_kf.filter_step(z_meas) # Kalman odda x_pred (poprzednią prędkość)
            
            R_curr = self.current_pose[0:2, 0:2]
            t_curr = self.current_pose[0:2, 2]
            prev_azimuth = np.arctan2(self.current_pose[1, 0], self.current_pose[0, 0])
            
            t_new_filtered = t_curr + R_curr @ np.array([filtered_deltas[0], filtered_deltas[1]])
            new_azimuth_filtered = prev_azimuth + filtered_deltas[2]
            ...
            step_is_valid = False # Ale ruch podtrzymany matematycznie!
        ...
        prev_azimuth_kf = np.arctan2(latest_kf_pose[1, 0], latest_kf_pose[0, 0])
        azimuth_diff = np.abs(np.arctan2(np.sin(global_azimuth - prev_azimuth_kf), np.cos(global_azimuth - prev_azimuth_kf)))
