...
class sonar_odometry(nn.Module):
    def __init__(self, model_config, sonar_config, device, 
            ...
            # ZMIANA: Inicjalizujemy procesor (do dekodowania wyników) i model
    ...
    def _match_and_estimate(self, ref_frame, ref_mask, ref_pose, ref_depth, new_frame, new_depth):
        ...
        # Wymuszamy rozmiar 832x832 do wykonania inferencji w RoPE
        target_size = (832, 832) 

        if ref_frame.shape[1] == 1:
            ref_frame_rgb = ref_frame.repeat(1, 3, 1, 1)
        else:
            ref_frame_rgb = ref_frame
        ...
        # ZMIANA: Dekodowanie gęstych map ciepła (heatmaps) za pomocą HF ImageProcessor
        # Informujemy procesor, że mapa ma wymiary 832x832.
        ...
        # Wyciągamy wyznaczone przez procesor gotowe punkty z tensorów CPU/GPU
        pts1_raw = processed_outputs.get('keypoints0', torch.empty((0, 2)).to(self.device)).to(self.device)
        pts2_raw = processed_outputs.get('keypoints1', torch.empty((0, 2)).to(self.device)).to(self.device)
        confidence = processed_outputs.get('matching_scores', torch.empty((0,)).to(self.device)).to(self.device)
...
#     def _match_and_estimate(self, ref_frame, ref_mask, ref_pose, ref_depth, new_frame, new_depth):
#         _, _, orig_h, orig_w = ref_frame.shape
#         # ZMIANA: Model wymaga wejścia w postaci 832x832, pod to wyliczana jest siatka RoPE.
#         target_size = (832, 832) 
...
#         # Powrót do oryginalnych wymiarów matematycznie
