...
class sonar_odometry(nn.Module):
    ...
    def __init__(self, model_config, sonar_config, device):
            ...
            self.depth_compesation = model_config.setup.use_depth_compensation
            ...
            self.bucket_grid = (model_config.filtering.bucket_grid, model_config.filtering.bucket_grid)
            self.max_pts_per_bucket = model_config.filtering.max_pts_per_bucket
            ...
            self.key_frames = model_config.keyframes.use
            self.window_size = model_config.keyframes.window_size
            self.key_frames_min_dist = model_config.keyframes.min_dist
            self.key_frames_min_rot = model_config.keyframes.min_rot
            self.key_frame_timeout = model_config.keyframes.max_skip_frames
            ...
            if self.input_format == 'polar':
                h = model_config.input.polar_height
                self.cart_frame_size = (h, 2 * h)
            else:
                self.cart_frame_size = (model_config.input.cart_height,
                                        model_config.input.cart_width)
    ...
    def fls_filter(self, frame):
        ...
        frame_torch = torch.tensor(filtered_float, device=device).unsqueeze(0).unsqueeze(0)
        return frame_torch
    ...
    def set_init_state(self, init_x, init_y, init_azimuth, init_frame, init_depth, carth_mask=None):
        ...
        self.sliding_window = [(first_frame, init_pose, init_depth)]
        self.last_frame_data = (first_frame, init_pose, init_depth)
    ...
    def forward(self, frame, depth, return_debug_data=False):
        ...
        if self.key_frames:
            ...
            # frame2frame odometry
            key_frame_detected = step_is_valid
        ...
        if not return_debug_data:
            ...
            if latest_visu_match is not None:
                ...
                pts1_visu = latest_visu_match['pts1']
                pts2_visu = latest_visu_match['pts2']
                conf_visu = float(np.mean(latest_visu_match['confidence']))
                ...
                v_raw_tx_sonar, v_raw_ty_sonar, v_raw_theta_sonar = 0.0, 0.0, 0.0
